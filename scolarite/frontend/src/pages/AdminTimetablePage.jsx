...
export default function AdminTimetablePage() {
  ...
  async function loadAll() {
    ...
    try {
      const [cRes, sRes, rRes] = await Promise.all([
        ...
        api.get("/admin/class-documents/timetable"),
      ]);
  ...
  async function submit(e) {
    ...
    try {
      ...
      fd.append("file", file);
      const res = await api.post("/admin/class-documents/timetable", fd, {
    ...
    <div className="admin-wrap">
      <aside className="admin-sidebar">
        ...
        <nav className="admin-nav">
          ...
          <Link className="admin-nav-item admin-nav-item--active" to="/directeur/timetable">{tr("Timetable", "Emploi du temps", "جدول التوقيت")}</Link>
          <Link className="admin-nav-item" to="/directeur/exam-calendar">{tr("Exam calendar", "Calendrier des examens", "رزنامة الامتحانات")}</Link>
      ...
      <main className="admin-main">
        <header className="admin-topbar">
          <div>
            <h1 className="admin-title">{tr("Timetable", "Emploi du temps", "جدول التوقيت")}</h1>
            <p className="admin-subtitle">{tr("Upload a PDF/image per class & semester, with start/end dates.", "Televersez un PDF/image par classe et semestre, avec dates debut/fin.", "ارفع ملف PDF/صورة لكل قسم وسداسي مع تاريخ بداية/نهاية.")}</p>
          </div>
        ...
        <section className="admin-card admin-card--padded" style={{ marginBottom: 18 }}>
          ...
          <form onSubmit={submit} className="admin-field-row" style={{ maxWidth: "none" }}>
            ...
            <div className="admin-field">
              <label className="admin-label">{tr("Start", "Debut", "البداية")}</label>
              <input className="admin-input" type="date" value={startsAt} onChange={(e) => setStartsAt(e.target.value)} />
            </div>
            <div className="admin-field">
              <label className="admin-label">{tr("End", "Fin", "النهاية")}</label>
            ...
            <div className="admin-field" style={{ maxWidth: "none" }}>
              ...
              <input className="admin-input" value={title} onChange={(e) => setTitle(e.target.value)} placeholder={tr("e.g. Timetable S1", "ex: Emploi du temps S1", "مثال: جدول السداسي 1")} />
            </div>
            <div className="admin-field" style={{ maxWidth: "none" }}>
        ...
        <section className="admin-card admin-card--padded">
          <h2 className="admin-card-heading">{tr("Published timetables", "Emplois du temps publies", "الجداول المنشورة")}</h2>
