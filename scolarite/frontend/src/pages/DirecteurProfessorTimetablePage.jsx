...
export default function DirecteurProfessorTimetablePage() {
  ...
  async function loadAll() {
    ...
    setError("");
    try {
      const rRes = await api.get("/directeur/professeur-documents/timetable");
  ...
  async function submit(e) {
    ...
    try {
      ...
      fd.append("file", file);
      const res = await api.post("/directeur/professeur-documents/timetable", fd, {
    ...
    <div className="admin-wrap">
      <aside className="admin-sidebar">
        ...
        <nav className="admin-nav">
          ...
          <Link className="admin-nav-item" to="/directeur/timetable">{tr("Timetable (students)", "Emploi du temps (etudiants)")}</Link>
          <Link className="admin-nav-item" to="/directeur/exam-calendar">{tr("Exam calendar (students)", "Calendrier examens (etudiants)")}</Link>
          <Link className="admin-nav-item admin-nav-item--active" to="/directeur/prof-timetable">{tr("Timetable (professors)", "Emploi du temps (professeurs)")}</Link>
          <Link className="admin-nav-item" to="/directeur/prof-exam-surveillance">{tr("Exam surveillance (professors)", "Surveillance examens (professeurs)")}</Link>
      ...
      <main className="admin-main">
        <header className="admin-topbar">
          <div>
            <h1 className="admin-title">{tr("Professor timetable", "Emploi du temps des professeurs")}</h1>
        ...
        <section className="admin-card admin-card--padded" style={{ marginBottom: 18 }}>
          ...
          <form onSubmit={submit} className="admin-field-row" style={{ maxWidth: "none" }}>
            ...
            <div className="admin-field" style={{ maxWidth: "none" }}>
              ...
              <input className="admin-input" value={title} onChange={(e) => setTitle(e.target.value)} placeholder={tr("e.g. Timetable", "ex: Emploi du temps")} />
            </div>
            <div className="admin-field" style={{ maxWidth: "none" }}>
