...
export default function StudentExamCalendarPage() {
  ...
  const loadAll = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await api.get("/student/class-documents/exam_calendar");
      setRows(Array.isArray(res.data) ? res.data : []);
    } catch (err) {
      setRows([]);
      setError(err?.response?.data?.message || tr("Could not load exam calendar.", "Impossible de charger le calendrier des examens.", "تعذر تحميل رزنامة الامتحانات."));
    } finally {
      setLoading(false);
    }
  }, [tr]);
  ...
  return (
    <div className="spn-wrap">
      <header className="spn-topnav">
        <div className="spn-topnav-inner">
          ...
          <nav className="spn-links">
            ...
            <Link to="/student/timetable">{t("menuSchedule")}</Link>
            <Link className="is-active" to="/student/exam-calendar">{t("menuExamCalendar")}</Link>
          </nav>
      ...
      <div className="spn-header">
        <div>
          <h1>{t("menuExamCalendar")}</h1>
          <p>{tr("The exam calendar is visible only until the end date set by the administration.", "Le calendrier des examens s'affiche uniquement jusqu'à la date de fin.", "تظهر رزنامة الامتحانات فقط إلى غاية تاريخ النهاية.")}</p>
        </div>
      ...
      {!loading && !error && rows.length === 0 && (
        <p className="spn-empty">{t("exNoDoc")}</p>
      )}
